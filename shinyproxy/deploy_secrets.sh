...
if [ "$#" -ne 6 ]; then
    echo "Usage: $0 POSTGRES_USERNAME POSTGRES_PASSWORD POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_DB_V2"
    exit 1
fi
...
POSTGRES_DB_V2=$(echo -n "${6}" | base64)
...
kubectl apply -f <( sed -e "s|\${POSTGRES_USERNAME}|${POSTGRES_USERNAME}|g" \
    ...
    -e "s|\${POSTGRES_DB_V2}|${POSTGRES_DB_V2}|g"  k8s-secrets.yaml )
